  ...
  constructor(options) {
    ...
    if (parent === null) {
      ...
      if (isFilteringByOnly) {
        if (this.only) {
          // Under isolation='none', isFilteringByOnly is forced on even without
          // --test-only, so a stray .only() silently filters out every other
          // test. Surface this once so it is not invisible in CI.
          // See https://github.com/nodejs/node/issues/60917.
          const { harness } = this.root;
          if (harness.isStrayOnlyFiltering && !harness.strayOnlyWarningEmitted) {
            harness.strayOnlyWarningEmitted = true;
            process.emitWarning(
              "A test using 'only' was found while running with " +
                '--test-isolation=none. Because --test-only was not passed, ' +
                "this 'only' is being honored and all other tests are being " +
                'filtered out. This behavior is not gated by --test-only under ' +
                'isolation=none. See https://github.com/nodejs/node/issues/60917.',
            );
          }
