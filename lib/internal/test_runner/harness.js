...
function createTestTree(rootTestOptions, globalOptions) {
  ...
  // Under isolation='none' (without a child test context), isFilteringByOnly is
  // forced to true regardless of --test-only. In that branch globalOptions.only
  // still mirrors getOptionValue('--test-only'), so a falsy value means
  // --test-only was not actually passed. When that happens, a stray .only()
  // silently filters out every other test with no CI-visible signal. Track the
  // condition so the first .only-driven filter can warn once.
  // See https://github.com/nodejs/node/issues/60917.
  const isStrayOnlyFiltering = isFilteringByOnly &&
  ...
  const harness = {
    ...
    isFilteringByOnly,
    isFilteringByTags,
    isStrayOnlyFiltering,
    strayOnlyWarningEmitted: false,
    async runBootstrap() {
